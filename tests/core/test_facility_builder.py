...
class MockGenData:
    def __init__(self, storage_capacity=1000):
...
def test_create_generator(setup_builder):
    ...
    assert generator.storage_capacity == 1000
...
def test_create_facility_capacity_limits(setup_builder, capacity):
    ...
    gen_data = MockGenData(storage_capacity=capacity)
    ...
    if capacity <= 0:
        ...
        generator = setup_builder.create_generator(gen_data, RegionType.OSREDNJESLOVENSKA)
        assert generator.storage_capacity == capacity
